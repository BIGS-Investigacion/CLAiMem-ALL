...
PAM50_LABELS = ['LumA', 'LumB', 'Her2-enriched', 'Basal', 'Normal']
IHC_LABELS = {
...
def load_class_distribution(csv_path: Path) -> pd.DataFrame:
    ...
    # Normalize label format to match metrics CSV
    # Map: luma->LumA, lumb->LumB, her2->Her2-enriched, basal->Basal, normal->Normal
    label_mapping = {
        'luma': 'LumA',
        'lumb': 'LumB',
        'her2': 'Her2-enriched',
...
def compute_prevalence(df_labels: pd.DataFrame, task: str = None) -> pd.DataFrame:
    """
    Compute class prevalence.

    Args:
        df_labels: DataFrame with slide_id and label columns
        task: Task name (for adding receptor prefix to IHC labels)
    ...
    # Add receptor prefix for IHC tasks
    if task and task in ['er', 'pr', 'her2']:
        receptor = task.upper()
        prevalence['Class'] = prevalence['Class'].apply(
            lambda x: f"{receptor}-{x}" if x in ['positive', 'negative'] else x
        )
...
def compute_prevalence_shift(prev_tcga: pd.DataFrame,
    ...
    # Compute shift metrics (signed difference, not absolute)
    df['Δp'] = df['Prevalence_CPTAC'] - df['Prevalence_TCGA']
    df['Δp_absolute'] = df['Δp']#np.abs(df['Δp'])
    df['Δp_relative_%'] = (df['Δp'] / df['Prevalence_TCGA']) * 100
...
def compute_performance_degradation(df_metrics: pd.DataFrame,
    ...
    # RPC = (HO - MCCV) / MCCV (signed)
    # Positive values = improvement (HO better than MCCV)
    # Negative values = degradation (HO worse than MCCV)
    df['RPC'] = (df[metric_ho] - df[metric_mccv]) / df[metric_mccv]
    df['RPC_%'] = df['RPC'] * 100

    # Absolute change for reference
...
def correlate_shift_and_degradation(df_shift: pd.DataFrame,
    ...
    """
    # Merge data - use signed Δp (not absolute)
    df = df_shift[['Class', 'Δp']].merge(
    ...
    if df['Δp'].nunique() == 1:
        # All prevalence shifts are identical
        return {
            'pearson_r': np.nan,
            'pearson_p': np.nan,
            'spearman_r': np.nan,
            ...
            'note': 'All prevalence shifts are identical - correlation undefined'
        }

    # Compute Pearson correlation
    r, p_value = stats.pearsonr(df['Δp'], df['RPC'])

    # Compute Spearman correlation (non-parametric)
    r_spearman, p_spearman = stats.spearmanr(df['Δp'], df['RPC'])
...
def plot_prevalence_shift(df_shift: pd.DataFrame, task: str,
                         output_path: Optional[Path] = None) -> None:
    """
    Plot signed prevalence shift.
    """
    fig, ax = plt.subplots(figsize=(10, 6))

    # Color by sign: red for negative (CPTAC < TCGA), green for positive (CPTAC > TCGA)
    colors = ['#d62728' if x < 0 else '#2ca02c' for x in df_shift['Δp']]

    bars = ax.bar(df_shift['Class'], df_shift['Δp'],
                  color=colors, alpha=0.7, edgecolor='black')

    # Add value labels
    for bar, delta in zip(bars, df_shift['Δp']):
    ...
    ax.set_xlabel('Class', fontsize=12, fontweight='bold')
    ax.set_ylabel('Δp (Signed Prevalence Shift)', fontsize=12, fontweight='bold')
...
def plot_correlation(corr_results: Dict, task: str,
    ...
    # Scatter plot
    scatter = ax.scatter(df['Δp'], df['RPC'],
    ...
    # Regression line (if correlation is defined)
    if not np.isnan(corr_results['regression_slope']):
        x_line = np.array([df['Δp'].min(), df['Δp'].max()])
    ...
    cbar.set_label('Performance Degradation (|RPC|)', fontsize=11)
...
def print_results(df_shift: pd.DataFrame, df_degradation: pd.DataFrame,
    ...
    print(f"\nMean Δp: {df_shift['Δp'].mean():.4f}")
    print(f"Max Δp:  {df_shift['Δp'].max():.4f} ({df_shift.loc[df_shift['Δp'].idxmax(), 'Class']})")
    print(f"Min Δp:  {df_shift['Δp'].min():.4f} ({df_shift.loc[df_shift['Δp'].idxmin(), 'Class']})")
    ...
    # Check for special case: all prevalence shifts identical
    if 'note' in corr_results:
        print(f"\n⚠ {corr_results['note']}")
        print("\n✓ CONCLUSION:")
        print("  For binary classification tasks, both classes have identical")
        print("  prevalence shifts by definition (complementary proportions).")
        print("  Correlation analysis is not applicable.")
        print("  Use overall prevalence shift and performance metrics instead.")
    elif np.isnan(r) or np.isnan(p):
        print("\n⚠ Correlation could not be computed (insufficient or invalid data)")
        print("\n✓ CONCLUSION:")
        print("  Insufficient data points or invalid values.")
        print("  Cannot assess distributional shift impact.")
    else:
        if abs(r) < 0.3:
            strength = "WEAK"
        elif abs(r) < 0.7:
...
Example:
  python distributional_shift_analysis.py \\
    ...
    parser.add_argument('--exclude_classes', '-e', type=str, nargs='+',
                        help='List of class names to exclude from analysis (e.g., Normal Her2-enriched)')
    ...
    prev_tcga = compute_prevalence(df_tcga, args.task)
    prev_cptac = compute_prevalence(df_cptac, args.task)
