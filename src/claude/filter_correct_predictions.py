...
def get_predicted_label_from_dir(image_dir):
    ...
    Examples:
        ...
        label_basal_pred_0 -> (0, 'basal')
        label_her2_pred_1 -> (1, 'her2')
    """
    dir_name = os.path.basename(os.path.normpath(image_dir))

    # Check if it's a PAM50 directory (has subtype name before pred)
    pam50_match = re.search(r'label_(\w+)_pred_(\d+)$', dir_name)
    if pam50_match:
        label_name = pam50_match.group(1)
        label_num = int(pam50_match.group(2))
        # PAM50 subtypes: basal, her2, luma, lumb, normal
        if label_name in ['basal', 'her2', 'luma', 'lumb', 'normal']:
            return label_num, label_name

    # Standard binary classification (negative/positive)
...
def filter_correct_predictions(image_dir, csv_path, output_dir=None, copy=False):
    ...
    predicted_label_num, predicted_label_name = get_predicted_label_from_dir(image_dir)
    if predicted_label_num is None:
        print(f"Error: Could not extract prediction label from directory name: {image_dir}")
        return [], [], []

    # Determine if this is PAM50 or binary classification
    is_pam50 = predicted_label_name is not None
    print(f"Task type: {'PAM50' if is_pam50 else 'Binary classification'}")
    print(f"Directory prediction label: {predicted_label_num}" +
          (f" ({predicted_label_name})" if is_pam50 else ""))
    ...
    for _, row in df.iterrows():
        ...
        if is_pam50:
            # For PAM50: keep text label, compare directly
            label_map[slide_id] = label
        else:
            # For binary: convert negative=0, positive=1
    ...
    for img_file in image_files:
        ...
        if is_pam50:
            # For PAM50: compare text label name
            is_correct = (predicted_label_name == true_label)
        else:
            # For binary: compare numeric label
            is_correct = (predicted_label_num == true_label)
